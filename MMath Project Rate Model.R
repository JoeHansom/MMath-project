...
prepare_data = function(df) {
  ...
  #standardise age (computational stability)
  df_clean$Age_std = (df_clean$Age - mean(df_clean$Age))/sd(df_clean$Age)
  
  # Adaptive spline fitting function
...
fit_model = function(stan_data, iter = 5000, warmup = 1000, chains = 4, 
  ...
  for (chain in 1:chains) {
    init_list[[chain]] = list(
      tau = rep(rnorm(1, 3.24, 0.5), stan_data$n_player),
      ...
      kappa = rnorm(1,1,0.05)
    )
  }
  
  cat("Initial Parameter Values:\n")
  for (chain in 1:chains) {
    cat(sprintf("  Chain %d: tau[1:3]=[%.3f,%.3f,%.3f], beta[1:3]=[%.3f,%.3f,%.3f], xi_1[1:3]=[%.3f,%.3f,%.3f], xi_2[1:3]=[%.3f,%.3f,%.3f]\n",
                chain,
                init_list[[chain]]$tau[1], init_list[[chain]]$tau[2], init_list[[chain]]$tau[3],
    ...
    cat(sprintf("      phi=%.3f, omega=%.3f, delta_inns=%.3f, delta_home=%.3f,kappa=%.3f\n",
                init_list[[chain]]$phi, 
                init_list[[chain]]$omega,
                init_list[[chain]]$delta_inns,
                init_list[[chain]]$delta_home,
                init_list[[chain]]$kappa))
  }
  ...
  fit = sampling(
    ...
    refresh = 100,
    pars = c("tau", "beta", "xi_1", "xi_2", "phi", "omega", "mu", "delta_inns", "delta_home","kappa", "log_lik", "y_rep")
  )
...
#model summary####
summary_report = function(results, n_tau = 10) {
...
rate_results = main(df)
saveRDS(rate_results, 'runs_results.rds')

summary_report(rate_results, n_tau = 20)
